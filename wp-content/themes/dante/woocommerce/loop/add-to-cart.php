...
<?php if ( ! $product->is_in_stock() ) : ?>
	...
	<a href="<?php echo apply_filters( 'out_of_stock_add_to_cart_url', get_permalink( $product_id ) ); ?>" class=""><i class="ss-info"></i><span><?php echo apply_filters( 'out_of_stock_add_to_cart_text', __( 'Read More', 'swiftframework' ) ); ?></span></a>
...
<?php else : ?>
	...
	<?php
		...
		echo apply_filters( 'woocommerce_loop_add_to_cart_link', sprintf('<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="%s product_type_%s"  data-added_text="%s">%s</a>', esc_url( $link['url'] ), esc_attr( $product_id ), esc_attr( $product->get_sku() ), esc_attr( $link['class'] ), esc_attr( $product_type ), $added_text, $link['label'] ), $product, $link);
